...
DACField DAC={true, true, 0, 1, {}, 0, 1, 100, 0, 0, 5, 100, 1};
			...
			*/
			if (abs((int)adc_in[0] - (int)old_adc_in[0]) > THRESHOLD) {
				...
				if (mean_amp == 1) {
					...
					//range checking
					if (fabs(CField.mean + temp) < 10 || fabs(CField.mean - temp) < 10) {
								printf("%dvvv\n", adc_in[0] - old_adc_in[0]);
						CField.amp = temp;
						hasChanged = true;
					}
				}
				//change mean if bit 2 is not set
					...
					//range checking
					if (fabs(CField.amp + temp) < 10 || fabs(CField.amp - temp) < 10) {
								printf("%dvvv\n", adc_in[0] - old_adc_in[0]);
						CField.mean = temp;
						hasChanged = true;
					}
