...
//Adding timespec
void addTime(struct timespec* time, struct timespec* sleep)
{
	time_t sec;
	long int nano_sec;

	nano_sec = time->tv_nsec + sleep->tv_nsec;
	sec = time->tv_sec + sleep->tv_sec + nano_sec/1000000000;
	nano_sec %= 1000000000;
	time->tv_sec = sec;
	time->tv_nsec = nano_sec;
}
...
void* PushDAC (void* Curr){
    ...
    struct timespec sleep, time_curr, time_end;
    int w=(int)f_available/(*Current).freq/(*Current).samples_per_period;
    float wf=f_available/(*Current).freq/(*Current).samples_per_period;

    //useconds_t usleeptime=(useconds_t) wf*100000/FBEST;
	//useconds_t usleeptime=(useconds_t) (float)1000000000/(Current->freq*Current->samples_per_period);
    ...
    while (1){
        for(i=0;i<(*Current).samples_per_period;i++) {
            ...
            //allow different DACs to write in turn
            	//addTime(&time_end, &sleep);
    			//pthread_mutex_lock(&PushMutex);
    			//pthread_cond_wait(&cond, &PushMutex);
        		...
        		while(1)
        		{
        			...
        			if (time_curr.tv_sec > time_end.tv_sec > 0)break;
        			else if(time_curr.tv_sec == time_end.tv_sec)
        				if(time_curr.tv_nsec >= time_end.tv_nsec)break;
        		};
        		*/
...
void showConfig(){
    ...
    printf("%*s%*d\t%*d\n", 25, "Running? (0-OFF, 1-ON)", 15, DAC[0].isOn, 15,
           DAC[1].isOn);
