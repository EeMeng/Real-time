...
void  INThandler(int sig){
    ...
    toReturn = true;
    signal(sig, SIG_IGN);
            //remove all input buffer before getchar
...
// Sleep function that continuous loops when CTRL+C is pressed
// It also loops once when CTRL+C is not pressed
void ctrlc_sleep(){
    do{
        delay(200);
    } while (ctrlc_pressed);
...
void showDACConfig(){
    ...
    printf("%*s%*.2E\n", 25,
           "DAC Output resolution (V)", 15, DAC.output_res);
